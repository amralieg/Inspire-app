...
const PRIORITY_SORT_ORDER = ['Ultra High', 'Very High', 'High', 'Medium', 'Low', 'Very Low'];

function priorityTierIndex(uc) {
  const p = uc.Priority || uc.priority || '';
  const i = PRIORITY_SORT_ORDER.indexOf(p);
  return i === -1 ? 99 : i;
}

/** Full quick-view shape from __inspire_usecases API rows */
function buildSummaryFromUclist(ucs) {
  if (!ucs || !ucs.length) return null;
  const domainNames = [
    ...new Set(
      ucs
        .map((u) => u._domain || u['Business Domain'] || u.business_domain || '')
        .filter(Boolean)
    ),
  ];
  const highCount = ucs.filter((uc) => HIGH_PRIORITIES.has(uc.Priority || uc.priority || '')).length;
  return {
    total: ucs.length,
    high: highCount,
...
/**
 * Per-session overview for list cards: uses `usecase_overview` from GET /api/inspire/sessions
 * (results_json is omitted on that endpoint), then falls back to embedded results_json if present.
 */
function getListRowOverview(session) {
  const api = session.usecase_overview;
  if (api && typeof api.total === 'number' && api.total > 0) {
    return {
      total: api.total,
      high: Number(api.high) || 0,
      domains: Number(api.domains) || 0,
      previewTitles: Array.isArray(api.preview_titles) ? api.preview_titles.filter(Boolean) : [],
    };
  }
  const legacy = getSessionSummary(session);
  if (legacy) {
    const previewTitles = [...legacy.useCases]
      .sort((a, b) => priorityTierIndex(a) - priorityTierIndex(b))
      .slice(0, 4)
      .map((uc) => uc.Name || uc.use_case_name || uc.name || uc.usecase || '')
      .filter(Boolean);
    return {
      total: legacy.total,
      high: legacy.high,
...
export default function ChoosePage({ settings, onNewExperiment, onNoData, onViewResults }) {
  ...
  const [quickViewUcsBySession, setQuickViewUcsBySession] = useState({});
  const [quickViewLoadingSid, setQuickViewLoadingSid] = useState(null);
  ...
  const fetchUcsForQuickView = useCallback(
    async (sessionId) => {
      const sid = String(sessionId);
      if (!inspireDatabase || !warehouseId) return;
      setQuickViewLoadingSid(sid);
      try {
        const headers = {};
        if (token) {
        ...
        const resp = await fetch(`/api/inspire/usecases?${q}`, { headers });
        if (resp.ok) {
          const data = await resp.json();
          setQuickViewUcsBySession((prev) => ({ ...prev, [sid]: data.usecases || [] }));
        }
      } catch {
        /* silent */
      } finally {
        setQuickViewLoadingSid((cur) => (cur === sid ? null : cur));
      }
    },
    [inspireDatabase, warehouseId, token, databricksHost]
  );
  ...
  const totalSummary = (() => {
    ...
    for (const s of completedSessions) {
      const row = getListRowOverview(s);
      if (row) {
        totalUc += row.total;
        totalHigh += row.high;
      }
    ...
    return totalUc > 0
      ? { total: totalUc, high: totalHigh, domains: allDomains.size > 0 ? allDomains.size : null }
      : null;
  })();
      ...
      <div className="relative z-10 max-w-3xl w-full space-y-8">
        ...
        <div className="text-center space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-[rgba(255,248,237,0.04)] border border-[rgba(255,248,237,0.08)] rounded-full">
            <BarChart3 size={12} className="text-[#FF3621]" />
          ...
          {totalSummary && (
            <p className="text-[11px] text-text-tertiary">
              {totalSummary.total} use cases discovered · {totalSummary.high} high priority
              {totalSummary.domains != null &&
                ` · ${totalSummary.domains} domain${totalSummary.domains !== 1 ? 's' : ''} explored`}
            </p>
          )}
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {(showAll ? sessions : sessions.slice(0, 5)).map((session) => {
              ...
              const mode = session.widget_values?.operation_mode
                || session.widget_values?.['15_operation']
                || session.widget_values?.['02_operation_mode']
                || session.operation_mode
                || '';
              const isCompleted = session.completed_percent >= 100;
              const sidStr = String(session.session_id);
              const legacySummary = getSessionSummary(session);
              const listOverview = getListRowOverview(session);
              const panelSummary = legacySummary || buildSummaryFromUclist(quickViewUcsBySession[sidStr]);
              const canQuickView = !!(legacySummary || listOverview?.total > 0);
              ...
              const isQuickOpen = String(expandedQuickView) === sidStr;
              ...
              return (
                ...
                >
                      ...
                      {listOverview && (
                        <>
                          <p className="text-[11px] text-text-secondary mt-1.5">
                            {listOverview.total} use case{listOverview.total !== 1 ? 's' : ''}
                            {' · '}
                            {listOverview.high} high priority
                            {' · '}
                            {listOverview.domains} domain{listOverview.domains !== 1 ? 's' : ''}
                            {isCompleted && session.completed_on
                              ? ` · Completed ${formatCompletedDate(session.completed_on)}`
                              : ''}
                          </p>
                          {listOverview.previewTitles.length > 0 && (
                            <p className="text-[10px] text-text-tertiary mt-1 line-clamp-2 leading-snug">
                              {listOverview.previewTitles.join(' · ')}
                              {listOverview.total > listOverview.previewTitles.length && (
                                <span>
                                  {' '}
                                  · +{listOverview.total - listOverview.previewTitles.length} more
                                </span>
                              )}
                            </p>
                          )}
                      ...
                      {canQuickView && (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            if (isQuickOpen) {
                              setExpandedQuickView(null);
                              return;
                            }
                            setExpandedQuickView(session.session_id);
                            if (!legacySummary && listOverview?.total > 0) {
                              const has = (quickViewUcsBySession[sidStr] || []).length > 0;
                              if (!has) fetchUcsForQuickView(session.session_id);
                            }
                          }}
                  ...
                  {/* ── Quick View Panel ── */}
                  {isQuickOpen && canQuickView && (
                    <div className="border-t border-border px-5 py-4 space-y-4">
                      {quickViewLoadingSid === sidStr && !panelSummary && (
                        <div className="flex items-center gap-2 text-sm text-text-secondary py-2">
                          <Loader2 size={14} className="animate-spin text-[#FF3621]" />
                          Loading use cases…
                        </div>
                      )}
                      {!legacySummary &&
                        quickViewLoadingSid !== sidStr &&
                        !panelSummary &&
                        listOverview?.total > 0 && (
                          <p className="text-xs text-text-tertiary py-1">
                            Preview could not be loaded. Open full results to browse all use cases.
                          </p>
                        )}
                      {panelSummary && (
                        <>
                      {/* Domain chips */}
                      {panelSummary.domainNames.length > 0 && (
                        <div className="flex flex-wrap gap-1.5">
                          {panelSummary.domainNames.map((d) => (
                      ...
                      <div className="space-y-1.5">
                        ...
                        {panelSummary.useCases.length > 8 && (
                          <p className="text-[10px] text-text-tertiary text-center pt-1">
                            +{panelSummary.useCases.length - 8} more use cases
                          </p>
                        )}
                      </div>
                        </>
        ...
        {error && (
          <div className="flex flex-col items-center gap-1 text-xs text-yellow-400/80 justify-center pt-2 px-4 text-center">
            <div className="flex items-center gap-2">
              <AlertCircle size={13} />
              <span>Could not load all sessions — you can still start a new experiment.</span>
            </div>
            {typeof error === 'string' && error.trim() && error !== 'false' && (
              <span className="text-[10px] text-text-tertiary font-mono break-all max-w-xl">{error}</span>
            )}
          </div>
        )}
      </div>
